...
POTD_MODEL = "claude-haiku-4-5-20251001"   # Fast + cheap for daily automation
...
# =========================
def generate_pick_of_the_day(df: pd.DataFrame, as_of_date: str) -> str:
    """
    Call the Anthropic API to reason over today's games and return a
    single Pick of the Day in the format: "TEAM - short reason"
    e.g. "COL - elite home record and well-rested after 3 days off"
    ...
    # Build a concise game summary to send to Claude
    tmp = df.copy()
    ...
    for _, row in tmp.iterrows():
        ...
        home_pts  = row.get("home_points_pct")
        away_pts  = row.get("away_points_pct")
        home_gd   = row.get("home_gd_per_g")
        away_gd   = row.get("away_gd_per_g")
        home_l10  = row.get("home_l10_points_pct")
        away_l10  = row.get("away_l10_points_pct")
        ...
        game_lines.append(
            f"{away}@{home} | Model pick: {pick} ({prob*100:.1f}%) | "
            f"Pts%: {away} {fs(away_pts,',.3f')} vs {home} {fs(home_pts,',.3f')} | "
            f"GD/G: {away} {fs(away_gd,'+.2f')} vs {home} {fs(home_gd,'+.2f')} | "
            f"L10 Pts%: {away} {fs(away_l10,',.3f')} vs {home} {fs(home_l10,',.3f')} | "
            f"Rest: {away} {fs(away_rest,'.0f')}d vs {home} {fs(home_rest,'.0f')}d"
        )

    games_block = "\n".join(game_lines)

    prompt = f"""You are an NHL betting analyst. Today is {as_of_date}.

Here are today's NHL games with model predictions and key team stats:

{games_block}

Your task: Choose ONE Pick of the Day. This should be the BEST VALUE bet — not necessarily the highest probability pick. Consider:
- Teams with strong recent form (L10 Pts%) even if overall season record is average
- Well-rested teams vs fatigued opponents
- Home ice advantage when stats are close
- Avoid picks where the model probability is very close to 50% (low conviction)
- Prefer picks where the model probability suggests genuine edge

Respond with ONLY this format (no other text, no explanation outside the reason):
TEAM_ABBREV - reason in 5 words or fewer

Example: COL - elite home record lately
Example: BOS - well-rested vs tired road team
Example: TBL - best recent form on slate"""
    ...
    try:
        ...
        if " - " in potd and len(potd) < 80:
            log(f"🌟 Pick of the Day: {potd}")
            return potd
        else:
            log(f"⚠️  POTD: unexpected format from API: {potd!r}")
            return ""
    except Exception as e:
        log(f"⚠️  POTD: API call failed — {e}")
        return ""
...
def run_for_date(
    ...
    if write_daily:
        ...
        # Generate Pick of the Day via Claude API, then write picks CSV
        potd = generate_pick_of_the_day(df, as_of_date=date_ymd)
        write_todays_picks_csv(df, as_of_date=date_ymd, potd=potd)
