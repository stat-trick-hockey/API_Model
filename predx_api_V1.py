...
# Pick of the Day — generated by Claude API reasoning over today's games
...
def render_potd_html(potd_short: str, potd_commentary: str) -> str:
    """Render the Pick of the Day card for the top of the HTML report."""
    if not potd_short:
        return ""

    # Split "TEAM - reason" into parts for display
    parts = potd_short.split(" - ", 1)
    team = parts[0].strip()
    reason = parts[1].strip() if len(parts) > 1 else ""

    commentary_block = f'<div class="potdCommentary">{potd_commentary}</div>' if potd_commentary else ""

    return f"""
    <div class="potdCard">
      <div class="potdLabel">⭐ Pick of the Day</div>
      <div class="potdTeam">{team}</div>
      <div class="potdReason">{reason}</div>
      {commentary_block}
      <div class="potdFooter muted">Generated by Claude · Based on model stats + form analysis</div>
    </div>
    """
...
# HTML
# =========================
def render_html(df: pd.DataFrame, date_ymd: str, tz_name: str, summary_html: str = "", spark_html: str = "", potd_html: str = "") -> str:
...
<head>
  ...
  <style>
    ...
    /* Pick of the Day */
    .potdCard {{ background: linear-gradient(135deg, rgba(255,200,50,.08) 0%, rgba(255,150,30,.05) 100%); border: 1px solid rgba(255,200,50,.25); border-radius:14px; padding:20px 24px; margin-bottom:18px; }}
    .potdLabel {{ font-size:11px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:rgba(255,200,50,.8); margin-bottom:8px; }}
    .potdTeam {{ font-size:36px; font-weight:900; letter-spacing:-.02em; color:#fff; line-height:1; }}
    .potdReason {{ font-size:14px; color:rgba(255,255,255,.6); margin-top:4px; margin-bottom:14px; font-style:italic; }}
    .potdCommentary {{ font-size:14px; color:rgba(255,255,255,.85); line-height:1.65; border-top:1px solid rgba(255,255,255,.08); padding-top:12px; margin-top:4px; }}
    .potdFooter {{ font-size:11px; margin-top:12px; }}
  </style>
</head>
<body>
  <div class="wrap">
...
# =========================
# PICK OF THE DAY (Claude API)
# =========================
def generate_pick_of_the_day(df: pd.DataFrame, as_of_date: str) -> tuple:
    """
    Two-phase POTD generation:
      Phase 1 — Research: one web search per team via Anthropic API + web_search tool,
                collecting injury reports, goalie starts, recent news for every team playing.
      Phase 2 — Analysis: Sonnet synthesizes research findings + model stats into
                a POTD pick and 2-3 sentence commentary.

    Returns (potd_short, potd_commentary). Both are empty string on failure.
    """
    api_key = os.getenv(ANTHROPIC_API_KEY_ENV, "")
    if not api_key:
        log("⚠️  POTD: ANTHROPIC_API_KEY not set — skipping Pick of the Day.")
        return "", ""

    if df is None or df.empty:
        return "", ""

    # --- Build stats block (shared by both phases) ---
    tmp = df.copy()
    ...
    def fs(v, fmt=".2f"):
        return f"{float(v):{fmt}}" if pd.notna(v) and v is not None else "N/A"
    ...
    for _, row in tmp.iterrows():
        ...
        p_home = row.get("p_home", 0.5)
        prob = float(p_home) if pick == home else 1.0 - float(p_home)
        game_lines.append(
            f"{away}@{home} | Pick: {pick} ({prob*100:.1f}%) | "
            f"Pts%: {away} {fs(row.get('away_points_pct'),',.3f')} / {home} {fs(row.get('home_points_pct'),',.3f')} | "
            f"GD/G: {away} {fs(row.get('away_gd_per_g'),'+.2f')} / {home} {fs(row.get('home_gd_per_g'),'+.2f')} | "
            f"L10 Pts%: {away} {fs(row.get('away_l10_points_pct'),',.3f')} / {home} {fs(row.get('home_l10_points_pct'),',.3f')} | "
            f"L10 GD/G: {away} {fs(row.get('away_l10_gd_per_g'),'+.2f')} / {home} {fs(row.get('home_l10_gd_per_g'),'+.2f')} | "
            f"PP%: {away} {fs(row.get('away_pp_pct'),'.1f')} / {home} {fs(row.get('home_pp_pct'),'.1f')} | "
            f"PK%: {away} {fs(row.get('away_pk_pct'),'.1f')} / {home} {fs(row.get('home_pk_pct'),'.1f')} | "
            f"SF/G: {away} {fs(row.get('away_sf_per_g'),'.1f')} / {home} {fs(row.get('home_sf_per_g'),'.1f')} | "
            f"SA/G: {away} {fs(row.get('away_sa_per_g'),'.1f')} / {home} {fs(row.get('home_sa_per_g'),'.1f')} | "
            f"Rest: {away} {fs(row.get('away_rest_days'),'.0f')}d / {home} {fs(row.get('home_rest_days'),'.0f')}d | "
            f"B2B: {away} {flag(row.get('away_b2b'))} / {home} {flag(row.get('home_b2b'))} | "
            f"3in4: {away} {flag(row.get('away_3in4'))} / {home} {flag(row.get('home_3in4'))}"
        )
    games_block = "\n".join(game_lines)
    ...
    # -------------------------------------------------------
    log("🧠 POTD analysis phase: synthesizing research + model stats...")

    analysis_prompt = f"""You are an NHL betting analyst. Today is {as_of_date}.

== TODAY'S GAMES — MODEL STATS ==
{games_block}

== TEAM RESEARCH (injuries, goalie starts, recent news) ==
{research_block}

== STAT GLOSSARY ==
- Pts%: Season points percentage. Higher = better overall team.
- GD/G: Goal differential per game. Best proxy for true team quality.
- L10 Pts%/GD/G: Last 10 games form. Captures hot/cold streaks.
- PP% / PK%: Power play and penalty kill. Critical in tight games.
- SF/G / SA/G: Shots for/against per game. Indicates zone control.
- Rest: Days since last game. B2B=0 days (major fatigue), 2+=well rested.
- B2B/3in4: Fatigue flags. B2B causes ~5% win rate drop, especially for goalies.

== ANALYSIS FRAMEWORK ==
1. INJURY/LINEUP IMPACT: Missing stars or a backup goalie on a B2B changes implied probability significantly.
2. FORM DIVERGENCE: L10 trending up = undervalued. L10 trending down = avoid despite good season stats.
3. FATIGUE MISMATCH: B2B team vs rested opponent is a concrete, quantifiable edge.
4. SHOT DOMINANCE: SF/G vs SA/G reveals teams that control play beyond just the scoreboard.
5. SPECIAL TEAMS EDGE: PP%/PK% gap is decisive in close games.
6. VALUE OVER CERTAINTY: A 62% pick at +120 beats a 75% pick at -320 from a value standpoint.

Choose ONE Pick of the Day — synthesizing BOTH the model stats AND the research findings.
If research reveals a key injury or backup goalie start, weight that heavily.

Respond in EXACTLY this format with nothing else:
PICK: TEAM_ABBREV - reason in 6 words or fewer
COMMENTARY: 2-3 sentences. Blend stats AND news — cite specific numbers AND any relevant injury/goalie intel. Sharp, confident analyst tone.

Example:
PICK: COL - elite GD, MIN backup goalie on B2B
COMMENTARY: Colorado's +0.82 GD/G leads the slate and they enter on 2 days rest. Minnesota is on a back-to-back with Fleury confirmed starting his second straight game — backup goalie on a B2B is a 6-8% edge in its own right. The model's 73% probability here aligns with the news."""
    ...
    try:
        resp = requests.post(
            ...
            json={
                "model": POTD_MODEL,
                "max_tokens": 300,
                "system": "You are an NHL betting analyst. Respond only in the exact format requested. No extra text before or after.",
                "messages": [{"role": "user", "content": analysis_prompt}],
            },
            timeout=45,
        )
        if not resp.ok:
            log(f"⚠️  POTD analysis error {resp.status_code}: {resp.text[:300]}")
            return "", ""
        ...
        for line in raw.splitlines():
            ...
            if line.upper().startswith("PICK:"):
                potd_short = line[5:].strip()
            elif line.upper().startswith("COMMENTARY:"):
                potd_commentary = line[11:].strip()

        if potd_short and " - " in potd_short:
            log(f"🌟 Pick of the Day: {potd_short}")
            log(f"   Commentary: {potd_commentary[:80]}...")
            return potd_short, potd_commentary
        else:
            log(f"⚠️  POTD: unexpected format from API: {raw!r}")
            return "", ""

    except Exception as e:
        log(f"⚠️  POTD analysis call failed — {e}")
        return "", ""
...
# =========================
def write_todays_picks_csv(df: pd.DataFrame, as_of_date: str, potd: str = "") -> None:
    """
    Write today's picks as a 4-row CSV — one column per game, sorted by confidence.
    Row 1 (team):  team abbreviations      e.g. COL, BOS, PIT ...
    Row 2 (ml):    fair ML for pick        e.g. -273, -187, -166 ...
    Row 3 (prob):  win probability         e.g. 73.2%, 65.1% ...
    Row 4 (potd):  Pick of the Day cell 1  e.g. "COL - elite home record lately"
    ...
    # POTD goes in pick_1 cell only — rest of the row is blank
    potd_row = [potd] + [""] * (len(picks) - 1)
    out = pd.DataFrame([picks, moneylines, win_probs, potd_row], columns=cols)
    out.insert(0, "row", ["team", "ml", "prob", "potd"])
...
def run_for_date(
    ...
    if write_daily:
        ...
        # Generate Pick of the Day via Claude API (returns short pick + commentary)
        potd_short, potd_commentary = generate_pick_of_the_day(df, as_of_date=date_ymd)
        potd_html = render_potd_html(potd_short, potd_commentary)
        ...
        # Write Tidbyt-friendly today's picks CSV (pass short pick for CSV row)
        write_todays_picks_csv(df, as_of_date=date_ymd, potd=potd_short)
